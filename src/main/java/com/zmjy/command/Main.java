...
public class Main {
    ...
    //远程地址
    private static final String remoteIp = "192.168.22.64";

    //心跳广播端口
    private static final Integer heartbeatBroadcastPort = 8081;

    //本机ip
    private static final String localIp = "192.168.22.64";
    //updIp
    private static final String updIp = "0.0.0.0";
    //upd端口
    private static final Integer updPort = 8081;
    //tpc-server端口
    private static final Integer tpcServerPort = 8083;
                    ...
                    case 2:
                        ...
                        if (assignCmd == 1) {
                            for (Mapping mapping : mappings.stream().collect(Collectors.toMap(
                                it -> StrUtil.concat(true, it.getNode().toString(), it.getSubNode().toString(), it.getFpId().toString()),
                                Function.identity(), (k1, k2) -> k1)).values()) {
                                tcpClient.send(FpIdBuild.fpState(mapping.getNode(), mapping.getSubNode(), mapping.getFpId()),
                                    "获取FP状态 node:" + mapping.getNode() + " sNode:" + mapping.getSubNode() + " fpId:" + mapping.getFpId());
                            }
                        } else if (assignCmd == 2) {
                            for (Mapping mapping : mappings.stream().collect(Collectors.toMap(
                                it -> StrUtil.concat(true, it.getNode().toString(), it.getSubNode().toString(), it.getFpId().toString()),
                                Function.identity(), (k1, k2) -> k1)).values()) {
                                tcpClient.send(FpIdBuild.logNozState(mapping.getNode(), mapping.getSubNode(), mapping.getFpId()),
                                    "获取所有逻辑油枪状态 node:" + mapping.getNode() + " sNode:" + mapping.getSubNode() + " fpId:"
                                        + mapping.getFpId());
                            }
                        }
                    case 0:
    ...
    public static List<Mapping> initMappings() {
        return ListUtil.toList(new Mapping(remoteIp, 1, 2, 1, 1, 1, 1, 95, new BigDecimal("7.68")),
            new Mapping(remoteIp, 1, 2, 1, 1, 2, 2, 98, new BigDecimal("8.67")));
    }
    ...
    public static void cDatInit(TcpClient tcpClient, List<Mapping> mappings) {
        ...
        Map<String, List<Mapping>> oilEngineGroup = mappings.stream()
            .collect(Collectors.groupingBy(it -> StrUtil.concat(true, it.getIp(), it.getNode().toString(), it.getSubNode().toString())));

        oilEngineGroup.forEach((oilEngine, oilEngineMappings) -> {
            Integer node = oilEngineMappings.get(0).getNode();
            Integer sNode = oilEngineMappings.get(0).getSubNode();
            ByteBuf buf = null;
            //油品分组
    ...
    public static void prIdInit(TcpClient tcpClient, List<Mapping> mappings) {
        ...
        oilEngineGroup.forEach((oilEngine, oilEngineMappings) -> {
            ...
            for (Integer prodNb : prodNbGroup.keySet()) {
                //设定油品编号
                buf = PrIdBuild.prodNb(node, sNode, prId, prodNb);
    ...
    public static void fpIdlnIdInit(TcpClient tcpClient, List<Mapping> mappings) {
        ...
        oilEngineGroup.forEach((oilEngine, oilEngineMappings) -> {
            ...
            fpIdLnIdGroup.forEach((prodNbFmId, prodNbMappings) -> {
                for (Mapping mapping : prodNbMappings) {
                    //设置逻辑油枪的油品
                    ByteBuf buf = FpIdLnIdBuild.prId(node, sNode, mapping.getFpId(), mapping.getLnId(), mapping.getProdNb());
    ...
    public static void sendHeartbeat(UpdServer updServer, List<Mapping> mappings) {
        ...
        new Thread(() -> {
            ...
            while (true) {
                try {
                    Thread.sleep(3000);
//                    C0 A8 04 E6 22 B8 02 01 01 00
                    mappings.stream()
                        .collect(Collectors.groupingBy(it -> StrUtil.concat(true, it.getIp(), it.getNode().toString(), it.getSubNode().toString())))
                        .forEach((oilEngine, oilEngineMappings) -> {
                            String ip = oilEngineMappings.get(0).getIp();
                            Integer node = oilEngineMappings.get(0).getNode();
                            Integer sNode = oilEngineMappings.get(0).getSubNode();
                            ...
                            pack[6] = (byte) (node & 0xFF);
                            pack[7] = (byte) (sNode & 0xFF);
                            pack[8] = 0x01;
                            pack[9] = 0x00;
                            ByteBuf buf = Unpooled.wrappedBuffer(pack);
                            String[] split = remoteIp.split("\\.");
//                            InetSocketAddress remoteAddress = new InetSocketAddress(StrUtil.concat(true, split[0], ".", split[1], ".", split[2], ".", "255"), heartbeatBroadcastPort);
                            InetSocketAddress remoteAddress = new InetSocketAddress("255.255.255.255", heartbeatBroadcastPort);
