...
public class FpIdTrDatTrSeqNbBuild {

    /**
     * 用来访问处在Payable（状态2）和Locked（状态3）的加油点的所有交易
     * 数据库: FP_ID (21H-24H) + TR_DAT (21H) + TR_Seq_Nb (0001-9999)
     * Data_id: 0B
     *
     * @param fpId 加油点标识(1-4)
     * @return {@link byte[] }
     */
    public static ByteBuf readAllTransaction(int subNode, int node, int fpId) {
        ...
        Data data = DataAddBuild.FpIdTrDatTrSeqNb20(
            ListUtil.toList(FpIdTrDatTrSeqNbEnums.TR_Seq_Nb, FpIdTrDatTrSeqNbEnums.TR_Amount, FpIdTrDatTrSeqNbEnums.TR_Volume,
                FpIdTrDatTrSeqNbEnums.TR_Unit_Price, FpIdTrDatTrSeqNbEnums.TR_Log_Noz), fpId);
     ...
     * @return {@link byte[] }
     */
    public static ByteBuf clearTransaction(int subNode, int node, int fpId, int trSeqNb) {
        ByteBuf buf = Unpooled.buffer();
        ...
        ByteBuf msg = Unpooled.buffer();
        //数据库地址长度：用于指明一个设备的数据库的地址的字节数 1字节
        Data data = DataAddBuild.FpIdTrDatTrSeqNb(FpIdTrDatTrSeqNbEnums.Clear_Transaction, fpId, trSeqNb);
        msg.writeByte(data.getDataAddLength());
